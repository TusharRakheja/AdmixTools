...
double printconfig(CONFIG *cpt, int mode, char *title) ;
int printtime()   ; 
...
//   printf(%12.6f %12.6f %12.6f\n", yscale, yyy, cpt -> dsig) ;
  ...
  printf("init: %9.3f\n", yscore) ;
  printf("number of distinct configurations (internal): %d\n", numconfighash) ;
  printbcl() ;

  for (k=0; k<numconfig; ++k) { 
   cpt = configlist[k] ; 
   printconfig(cpt, 2, "initconfig:") ;
  }
  printnl() ; 
...
//  printf("bigiter:: %d\n", bigiter) ;  fflush(stdout) ; 
  ...
  for (k=0; k<numconfig; ++k) { 
   cpt = configlist[k] ;
   zz = printconfig(cpt, 2, "config:") ;
   zmax = MAX(zmax, fabs(zz)) ;
  }
  printnl() ;

  if (quitearly) bigiter = 0 ;
 ...
 if (bigiter>0) {
  ...
  for (iter = 1; iter <= bigiter; ++iter) { 
   ...
   printf("iter: %3d score: %12.6f worst absolute Z: %9.3f\n", iter, yscore, zmax) ;
   printnl() ;

   sciter[iter] = yscore ; 
   if (yscore < (sciter[iter-1] + scbreak)) { 
    printf("score increase  is small.  Terminating\n") ;
    break ; 
   }
   if (zmax < zbreak) {  
    printf("zmax is small.  Terminating\n") ;
    break ;
   }
   gslfree() ;
   dumpgraph (graphoutname);
  ...
  for (k=0; k<numconfig; ++k) { 
   cpt = configlist[k] ;
   zz = printconfig(cpt, 2, "finalconfig:") ;
   zmax = MAX(zmax, fabs(zz)) ;
  }
  getvpars(vpars) ; 
  putgpars(vpars) ; 
  printf("finalscore: %12.6f worst absolute Z: %9.3f\n", yscore, zmax) ;
  printnl() ;
  ...
  printf("##end of kimf: %12.3f seconds cpu %12.3f Mbytes in use\n", cputime(1), ymem) ;
  return 0;
...
void getvpars(double *vp)  
{
  ...
  for (k=0; k<npars; ++k) { 
   ...
   if (tt==1)  {
      ...
      z = logpt -> pop1 ; 
      f = logpt -> isfixedr ;  
      if (f == NO) logpt -> tau2 = vp[k]  ;
   }

   if (tt==2)  {
      t -= 2000 ;
      logpt = loglist[t] ;  
      if (logpt->isfixed == NO) logpt -> tmix = vp[k]  ;
   }
  ...
  for (k=0; k<npars; ++k) {
   ...
   switch (tt) {
    ...
    case 2:
      ...
      b = logpt -> pop2 ;
      c = logpt -> pop3 ;
      if (logpt -> isfixed == NO) setapar(a, b, c, y) ; 
      break ;
...
//  if (t<0) return ;   fixed but we want that
    if (t<0) logpt -> isfixed = YES ;  
...
{
  char *spt[MAXFF], *sx, *s2[20] ;
  int nsplit, nasc, k, z, xd, xt, n, t1, t2 ;
  LOGENTRY *logpt ;
  int p1, p2 ; 
  double tau1, tau2 ; 
  double th1, th2 ;
   ...
   logpt -> thetaval1 = th1 ; 
   logpt -> thetaval2 = th2 ; 
   logpt -> isfixedl  = isfixed(z, YES) ; 
   logpt -> isfixedr  = isfixed(z, NO) ; 
...
readcommands (int argc, char **argv)
{
  ...
  getint(ph, "hashsize:", &hashsize) ;
  getint(ph, "hashmode:", &hashmode) ;
  getint(ph, "quitearly:", &quitearly) ;
  getdbl(ph, "zbreak:", &zbreak) ;
// quit if Z < zbreak
  getdbl(ph, "scbreak:", &scbreak) ;
// quit if score increase < scbreak
...
double printconfig(CONFIG *cpt, int mode, char *title) 
{
   ...
   if (mode >= 1) { 
    ...
    y2 = cpt -> dsig ;
    z = y1/(y2+1.0e-8) ;
    printf(" %9.3f", z) ;   // Z score 
    printf(" :: %12.6f %12.6f", y1, y2) ;
    printnl() ; 
    return z ; 
   }
